        ...
        function getInfo(scope, attrs) {
            ...
            function getEntityAndPropertyPaths() {
                ...
                if (ngModel) {
                    ...
                    if(ngModel.indexOf('[')){
                        // ex: 'activeItem[formField.key]'
                        // propertyPath should be the interpreted result of 'formField.key'
                        // entityPath should be 'activeItem'
                        var str = ngModel.replace(']','');
                        paths = str.split('[');
                        var fn = $parse(paths[1])
                        propertyPath = fn(scope);
                        entityPath = paths[0];
                    }else{
...
//                element.attr('required', true);
    ...
    module.provider('zDirectivesConfig', function() {
        ...
        this.zRequiredTemplate =
            '<span class="error-required glyphicon glyphicon-asterisk" title="Required"></span>';
